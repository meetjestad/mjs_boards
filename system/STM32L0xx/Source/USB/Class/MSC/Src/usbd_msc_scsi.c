...
static void SCSI_DoPrepareReceive(USBD_HandleTypeDef *pdev)
{ 
...
static void SCSI_DoTransmit(USBD_HandleTypeDef *pdev, uint32_t size)
{
    USBD_MSC_BOT_HandleTypeDef  *hmsc = (USBD_MSC_BOT_HandleTypeDef*)pdev->pClassData[1];  
...
static int8_t SCSI_TestUnitReady(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->IsReady(lun) !=0 )
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t  SCSI_Inquiry(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  else
  {
    ...
    pPage = (const uint8_t *)&((const USBD_StorageTypeDef *)pdev->pUserData[1])->pInquiry[lun * STANDARD_INQUIRY_DATA_LEN];
    len = pPage[4] + 5;
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->GetCapacity(lun, &hmsc->scsi_blk_nbr, &hmsc->scsi_blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_ReadFormatCapacity(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  ...
  if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->GetCapacity(lun, &blk_nbr, &blk_size) != 0)
  {
    SCSI_SenseCode(pdev,
                   lun,
                   NOT_READY, 
...
static int8_t SCSI_StartStopUnit(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  USBD_MSC_BOT_HandleTypeDef  *hmsc = (USBD_MSC_BOT_HandleTypeDef*) pdev->pClassData[1];   
  hmsc->bot_data_length = 0;
  ((const USBD_StorageTypeDef *)pdev->pUserData[1])->StartStopUnit(lun, (params[4] >> 0) & 1, (params[4] >> 1) & 1);
  return 0;
}
...
static int8_t SCSI_AllowMediumRemoval(USBD_HandleTypeDef  *pdev, uint8_t lun, uint8_t *params)
{
  USBD_MSC_BOT_HandleTypeDef  *hmsc = (USBD_MSC_BOT_HandleTypeDef*) pdev->pClassData[1];   
  hmsc->bot_data_length = 0;
  ((const USBD_StorageTypeDef *)pdev->pUserData[1])->PreventAllowMediumRemoval(lun, params[4]);
  return 0;
}
...
static int8_t SCSI_Read10(USBD_HandleTypeDef  *pdev, uint8_t lun , uint8_t *params)
{
  ...
  if(hmsc->bot_state == USBD_BOT_IDLE)  /* Idle */
  {
    ...
    if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->IsChanged(lun) !=0 )
    {
      SCSI_SenseCode(pdev,
                     lun,
                     UNIT_ATTENTION, 
    ...
    if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->Acquire(lun) !=0 )
    {
      SCSI_SenseCode(pdev,
                     lun,
                     NOT_READY, 
...
static int8_t SCSI_Write10 (USBD_HandleTypeDef  *pdev, uint8_t lun , uint8_t *params)
{
  ...
  if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
  {
    ...
    if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->IsWriteProtected(lun) !=0 )
    {
      SCSI_SenseCode(pdev,
                     lun,
                     NOT_READY, 
    ...
    armv6m_svcall_1((uint32_t)&SCSI_DoPrepareReceive, (uint32_t)pdev);
  }
...
static int8_t SCSI_ProcessRead (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  if( ((const USBD_StorageTypeDef *)pdev->pUserData[1])->Read(lun ,
							      hmsc->bot_data, 
							      hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
							      len / hmsc->scsi_blk_size,
							      (hmsc->scsi_blk_len == len)) < 0)
  {
  ...
  armv6m_svcall_2((uint32_t)&SCSI_DoTransmit, (uint32_t)pdev, len);
...
static int8_t SCSI_ProcessWrite (USBD_HandleTypeDef  *pdev, uint8_t lun)
{
  ...
  if(((const USBD_StorageTypeDef *)pdev->pUserData[1])->Write(lun ,
							      hmsc->bot_data, 
							      hmsc->scsi_blk_addr / hmsc->scsi_blk_size, 
							      len / hmsc->scsi_blk_size,
							      (hmsc->scsi_blk_len == len)) < 0)
